...
class SettingsFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        // ---------- 设置项：每项点击弹出二级设置弹窗 ----------
        root.findViewById<View>(R.id.row_trade).setOnClickListener { showTradeDialog(st) }
    ...
    /** 切换应用主题；红色(mode==3)恒为浅色+红色品牌，浅/深色走系统夜间模式。 */
    ...
    private fun showTradeDialog(st: com.quantapp.trader.trading.AppStore) {
    ...
    private fun showThemeDialog() {
        val ctx = requireContext()
        val st = App.appStore
        val v = LayoutInflater.from(ctx).inflate(R.layout.dialog_settings_theme, null)
        val rg = v.findViewById<RadioGroup>(R.id.rg_theme_dialog)
        val rbLight = v.findViewById<RadioButton>(R.id.rb_theme_light_dialog)
        val rbDark = v.findViewById<RadioButton>(R.id.rb_theme_dark_dialog)
        val rbRed = v.findViewById<RadioButton>(R.id.rb_theme_red_dialog)
        when (st.themeMode) {
            1 -> rbLight.isChecked = true
            2 -> rbDark.isChecked = true
            3 -> rbRed.isChecked = true
            else -> v.findViewById<RadioButton>(R.id.rb_theme_sys_dialog).isChecked = true
        }
        rg.setOnCheckedChangeListener { _, id ->
            applyTheme(when (id) {
                R.id.rb_theme_light_dialog -> 1
                R.id.rb_theme_dark_dialog -> 2
                R.id.rb_theme_red_dialog -> 3
                else -> 0
            })
        }
        openSaveDialog(v, saveText = "完成") { }
    ...
    /** 系统更新：显示当前版本并提供“检查更新”。 */
    private fun showSystemUpdateDialog(st: com.quantapp.trader.trading.AppStore) {
        val ctx = requireContext()
        val v = LayoutInflater.from(ctx).inflate(R.layout.dialog_settings_update, null)
        val tvVersion = v.findViewById<TextView>(R.id.tv_version_dialog)
        val btnCheck = v.findViewById<Button>(R.id.btn_check_update_dialog)
        tvVersion.text = "v${BuildConfig.VERSION_NAME} (${BuildConfig.VERSION_CODE})"
        btnCheck.setOnClickListener {
            if (st.updateUrl.isEmpty()) {
                Toast.makeText(ctx, "未配置升级地址", Toast.LENGTH_SHORT).show()
            ...
            scope.launch {
                try {
                    ...
                    Toast.makeText(ctx, "检查更新失败：${e.message}", Toast.LENGTH_LONG).show()
                } finally {
    ...
    private fun updateHint(live: Boolean, tv: TextView) {
        tv.text = if (live)
            "实盘信号模式：引擎只输出买卖信号并POST到网关，不直接成交，网关请在【实盘设置】中配置。"
